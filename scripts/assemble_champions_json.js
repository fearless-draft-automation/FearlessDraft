...
async function fetchChampions() {
    const response = await fetch("https://cdn.communitydragon.org/latest/champions");
    return await response.json();
}

function filterWildRift(champions) {
    const filepath = path.join(__dirname, "../priv/non_wild_rift_champtions.json")
...
async function addPositions(champions) {
    ...
    const positionsLookup = Object.entries(playRates)
        .reduce((acc, elem) => {
...
function determinePositions(positionPlayRates) {
    return Object.entries(positionPlayRates)
        .reduce((acc, elem) => {
